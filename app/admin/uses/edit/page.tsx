...
async function saveAuthorData(data: Author) {
  ...
  const response = await fetch('/api/about?type=usage', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error('Failed to save author usage');
  }
...
export default function EditAuthorPage() {
  ...
  useEffect(() => {
    async function loadAuthorData() {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to load author usage');
        }
        ...
        toast.error(`Failed to load author usage: ${error}`, toastOptions);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success('Author usage saved successfully', toastOptions);
      router.push('/admin/uses');
    } catch (err) {
      toast.error(`Failed to save author usage: ${err}`, toastOptions);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="container mx-auto py-10">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/admin/about">Uses</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Uses - Edit</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <Card className="mx-auto mt-2 max-w-5xl">
        <CardHeader>
          <CardTitle className="text-3xl">Edit Author Usage</CardTitle>
          <CardDescription>Update your author usage</CardDescription>
        </CardHeader>
