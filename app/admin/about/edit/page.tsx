...
async function saveAuthorData(data: Author) {
  ...
  const response = await fetch('/api/about?type=default', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error('Failed to save author data');
  }
...
export default function EditAuthorPage() {
  ...
  useEffect(() => {
    async function loadAuthorData() {
      try {
        ...
        toast.error(`Failed to load author data: ${error}`, toastOptions);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success('Author data saved successfully', toastOptions);
      router.push('/admin/about');
    } catch (err) {
      toast.error(`Failed to save author data: ${err}`, toastOptions);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="container mx-auto py-10">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/admin/about">About</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
      ...
      <Card className="mx-auto mt-2 max-w-5xl">
        <CardHeader>
          <CardTitle className="text-3xl">Edit Author Information</CardTitle>
          <CardDescription>Update your author details</CardDescription>
        </CardHeader>
