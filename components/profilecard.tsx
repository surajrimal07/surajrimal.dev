...
export const ProfileCard = () => {
      ...
      {languages.map((lang) => (
        <div key={lang} className="mb-2">
          ...
          <div className="h-2 w-full rounded-full bg-gray-200">
            <div className="h-2 rounded-full bg-red-500" style={{ width: languagePercentages[lang] }}></div>
          </div>
        </div>
      ))}
    </div>
  ...
  const renderGithubStats = () => (
    <>
      <h4 className="w-full mb-2 font-semibold">Github Stats</h4>
      <div className="flex w-full justify-between">
        <StatItem icon={<FaRegStar size={24} />} value={githubStats.totalStars} />
        <StatItem icon={<RiGitRepositoryLine size={24} />} value={userData?.public_repos || 0} />
        <StatItem icon={<RiGitRepositoryPrivateLine size={24} />} value={githubStats.privateRepos} />
        <StatItem icon={<FaUserFriends size={24} />} value={userData?.followers || 0} />
        <StatItem icon={<FaUserPlus size={24} />} value={userData?.following || 0} />
      </div>
      <h4 className="w-full mb-2 font-semibold">Last Commit: {githubStats.lastCommitDate}</h4>
    </>
  );

  return (
    <div className="flex flex-col items-center space-y-4 rounded-lg bg-gray-100 p-6 dark:bg-black border border-gray-300 dark:border-gray-700">
      <Image
